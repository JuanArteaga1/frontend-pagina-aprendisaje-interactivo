...
const Navbar = () => {
  ...
  // Manejo de mouse enter y leave
  const handleMouseEnter = () => {
    clearTimeout(timeoutId); // Cancelar cualquier timeout anterior
    if (!isDropdownOpen) setIsDropdownOpen(true); // Solo abrir si no está abierto por clic
  };

  const handleMouseLeave = () => {
    if (!isDropdownOpen) { // Solo cerrar si no está abierto por clic
  ...
  const handleClick = () => {
    setIsDropdownOpen(!isDropdownOpen); // Alternar estado al hacer clic
  };

  const handleOutsideClick = (e) => {
    if (isDropdownOpen && !e.target.closest(".dropdown-container")) {
      setIsDropdownOpen(false); // Cerrar si el clic es fuera del dropdown
    }
  };
      ...
      <div className="flex gap-6 mt-4 md:mt-0 ml-auto items-center">
        ...
        >
          <button
            ...
            onClick={handleClick} // Se activa al hacer clic
          >
