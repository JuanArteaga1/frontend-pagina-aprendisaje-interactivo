...
export const ProyectosProvider = ({ children }) => {
    ...
    const TraerProyectos = async () => {
        try {
            const proyectos = await GetAllProyectos(); // Llamada a la API
            console.log(proyectos); // Imprimir respuesta para depuración
    ...
    return (
        <ProyectoContext.Provider value={{ sigout, Proyectos, errors, mensaje, setMensaje, TraerProyectos }}>
            {children}
